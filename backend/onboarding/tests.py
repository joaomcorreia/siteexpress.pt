...
class OnboardingFlowTests(TestCase):
    ...
    def test_assistant_instructions_use_plain_language_and_honest_preview_flow(self):
        ...
        self.assertIn("pré-visualização", ASSISTANT_INSTRUCTIONS)
        self.assertIn("Nunca diga que o preview só está disponível no botão Começar", ASSISTANT_INSTRUCTIONS)
    ...
    @override_settings(SITEEXPRESS_ASSISTANT_MODE="openai", OPENAI_API_KEY="test-key")
    @patch("onboarding.views.generate_assistant_reply")
    @patch("onboarding.views.analyze_assistant_build")
    def test_assistant_creates_private_visual_build_when_business_is_ready(
        self, mock_analysis, mock_reply
    ):
        mock_reply.return_value = {
            "text": "Já percebi o essencial do seu negócio.",
            "mode": "openai",
            "model": "test-model",
        ...
        mock_analysis.return_value = {
            "ready": True,
            "business_name": "",
            "business_type": "Pintura de interiores",
            "category": "construction",
            "location": "",
            "headline": "Pintura cuidada para cada espaço",
            "intro": "Serviços de pintura para casas, apartamentos e escritórios.",
            "services": [
                {"title": "Pintura de interiores", "description": "Paredes e tetos."},
                {"title": "Preparação", "description": "Pequenas reparações antes da pintura."},
            ],
            "about": "Uma proposta simples para apresentar os serviços de pintura.",
            "cta": "Pedir orçamento",
        }

        response = self.client.post(
            reverse("assistant-chat"),
            data=json.dumps({"message": "Sou pintor e pinto casas e apartamentos."}),
            content_type="application/json",
        )
        ...
        confirmation = self.client.post(
            reverse("assistant-chat"),
            data=json.dumps({
                "message": "Sim, quero ver",
                "conversation_id": payload["conversation_id"],
            }),
            content_type="application/json",
        )
        self.assertEqual(confirmation.status_code, 200)
        confirmed_payload = confirmation.json()
        self.assertTrue(confirmed_payload["site_build"]["auto_start"])
        build.refresh_from_db()
        self.assertEqual(build.status, AssistantSiteBuild.Status.READY)

        build_page = self.client.get(confirmed_payload["site_build"]["url"])
        preview = self.client.get(
            reverse("assistant-site-build-preview", args=[build.public_id]),
            {"stage": 3},
        )
    ...
    def test_assistant_applies_feedback_to_existing_visual_build(self, mock_revision):
        ...
        mock_revision.return_value = {
            ...
            "location": "", "headline": "Uma casa com novas cores", "intro": "Pintura para a sua casa.",
            "services": [{"title": "Interiores", "description": "Paredes."}], "about": "Pintura cuidada.",
            "cta": "Pedir orçamento",
        }
    ...
    def test_assistant_visual_build_is_not_available_to_another_session(self):
        ...
        response = self.client.get(reverse("assistant-site-build", args=[build.public_id]))
    ...
    def test_starter_preview_includes_anchor_menu_for_services_family(self):
        ...
        self.assertContains(
            response,
            'class="card cta-card section-emphasis services-cta-card quote-section-ready section-ready" id="quote"',
            html=False,
        )
